use super::plugin_response_conversions::*;
use super::*;
...
impl PluginRequestProcessor {
    ...
    async fn plugin_installed_response(
        ...
        let (mut data, mut marketplace_load_errors) =
            Self::list_installed_plugin_marketplace_entries(
        ...
        if !install_suggestion_plugin_names.is_empty() {
            let (suggestion_data, suggestion_errors) =
                Self::list_install_suggestion_plugin_marketplace_entries(
                    plugins_manager.clone(),
                    plugins_input,
                    roots,
                    install_suggestion_plugin_names,
        ...
        if config.features.enabled(Feature::RemotePlugin) {
            Self::merge_remote_installed_plugin_marketplace_entries(
                &mut data,
                plugins_manager,
                &config,
                auth.as_ref(),
    ...
    async fn list_installed_plugin_marketplace_entries(
    ...
    > {
        ...
        tokio::task::spawn_blocking(move || {
            let outcome = plugins_manager.list_installed_plugins_for_config(&plugins_input);
            (
                outcome
                    .marketplaces
                    .into_iter()
                    ...
                    .collect(),
                convert_marketplace_load_errors(outcome.errors),
            )
        })
        .await
        .map_err(|err| internal_error(format!("failed to list installed plugins: {err}")))
    }

    async fn list_install_suggestion_plugin_marketplace_entries(
        plugins_manager: Arc<codex_core_plugins::PluginsManager>,
        plugins_input: codex_core_plugins::PluginsConfigInput,
    ...
    > {
        let shared_plugin_ids_by_local_path = load_shared_plugin_ids_by_local_path(config)?;
        match tokio::task::spawn_blocking(move || {
            let outcome = plugins_manager.list_marketplaces_for_config(&plugins_input, &roots)?;
            Ok::<
                (
                    Vec<PluginMarketplaceEntry>,
                    Vec<codex_app_server_protocol::MarketplaceLoadErrorInfo>,
    ...
    async fn merge_remote_installed_plugin_marketplace_entries(
            ...
            plugins_manager.cached_remote_installed_marketplaces()
        {
            ...
            plugins_manager
                .fetch_cached_remote_installed_marketplaces(&remote_plugin_service_config, auth)
                .await
        };
        ...
        match remote_marketplaces {
            Ok(remote_marketplaces) => {
                for remote_marketplace in remote_marketplaces
                    ...
                    .map(remote_marketplace_to_info)
                {
                    merge_plugin_marketplace_entry(data, remote_marketplace);
                }
            }
            Err(
                RemotePluginCatalogError::AuthRequired
